...
namespace msgframe {
    ...
    void MessageFrame::serialize(std::vector<uint8_t>& output_buffer) const {
        ...
        // Top-level structure: Fixed array of 3 elements [Header, Parameters, Attachments]
        pk.pack_array(3);

        // 1. Pack the header
        m_header.pack(&pk);

        // 2. Pack the parameter map
        parameters.pack(&pk);

        // 3. Pack binary attachments
        pk.pack_array(attachments.size());
        for (const auto& attach : attachments) {
            pk.pack_array(2); // Pair: [Name, Data]
            pk.pack(attach.name);

            // Write large byte array in MessagePack BIN format
            pk.pack_bin(attach.raw_data.size());
            pk.pack_bin_body(reinterpret_cast<const char*>(attach.raw_data.data()), attach.raw_data.size());
        }
    }

    // Deserialization of the entire frame from a raw network buffer
    bool MessageFrame::deserialize(const uint8_t* data, size_t size) {
        if (!data || size == 0) return false;

        try {
            // Parse the MessagePack object tree "on the fly"
            msgpack::object_handle oh = msgpack::unpack(reinterpret_cast<const char*>(data), size);
            msgpack::object const& root_obj = oh.get();

            // Root element must be an array of 3 elements
            ...
            // Protection against crashes in case of corrupted binary packet
            return false;
        }
    }
} // namespace msgframe
