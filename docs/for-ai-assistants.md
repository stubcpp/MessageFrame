...
* **Buffer & container reuse:** In tight execution loops, do **not** re-instantiate `MessageFrame`. Call `msg.clear()` at the end of each iteration. `clear()` always releases the current storage and re-applies the original `FrameConfig`. Without a hint, that means vector mode again. With `FrameConfig::initial_reserve > 128`, it goes straight back into a freshly-sized map instead of re-growing from a vector — so the *next* fill skips the migration step, even though the underlying allocation itself is recreated, not reused in place.
